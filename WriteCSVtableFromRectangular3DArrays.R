...
# ------------------------------------------------
# Writes data from the four (or five) 3D arrays representing xcoordinates, ycoordinates,
# zcoordinates and vcoordinates into a CSV file which can
# be imported to Voxler. CSV has the following four columns: xcoord,
# ycoord, zcoord (time in seconds), vcoord (density value) and wcoord (density x activity probability).
# ------------------------------------------------
...
WriteCSVtableFromFiveRectangular3DArrays <- function(xc,yc,zc,vc,wc,FileName) {
  ...
  # Build an empty CSV table of correct dimensions
  CSVtable <- array(data=0,dim=c(rowsX*colsX*sheetsX,5))
  colnames(CSVtable) <- c('X','Y','T','density','density x actProb')
  ...
  for (k in 1:sheetsX) {
    ...
    for (i in 1:rowsX) { 
      for (j in 1:colsX) {
        # print(c(i,j,k))
        CSVtable[(k-1)*rowsX*colsX+(i-1)*colsX+j,] <- c(xc[i,j,k],yc[i,j,k],zc[i,j,k],vc[i,j,k],wc[i,j,k])
      } # j
    } # i
  } # k 
